...
final class MasterClockTests: XCTestCase {
    func testNoDriftExactFireCount() throws {
    ...
    func testBandLadderSixToOne() throws {
    ...
    func testGearCompositionIsExact() throws {
        // (1/6) of (1/6) == 1/36, reduced and printed
        let a = try GearRatio(1, over: 6)
        XCTAssertEqual(try a.composed(with: a), try GearRatio(1, over: 36))
        XCTAssertEqual(try GearRatio(4, over: 6), try GearRatio(2, over: 3)) // reduction
        XCTAssertEqual(try GearRatio(2, over: 3).description, "2/3")
    }

    func testLatchCapturesExactTickAndValue() throws {
    ...
    func testOverdrivenGearMultiFires() throws {
        // p > q: a gear may fire more than once per master tick, still exact
        var g = PhaseGear(name: "fast", ratio: try GearRatio(5, over: 2))
        var clock = MasterClock()
        clock.advance()
