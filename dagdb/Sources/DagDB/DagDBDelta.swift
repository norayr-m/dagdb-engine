...
public struct DagDBDelta {
    ...
    public final class Encoder {
        ...
        public init(path: String, nodeCount: Int, maxRank: Int,
            ...
            // Post-u32-widen (2026-04-20): state.rank is u32 internally; .dagb format
            // stays u8 for compat with Savanna-era recordings. Truncates to
            // 255; the time-series codec doesn't exercise larger ranks.
            let rankU8 = staticState.rank.map { UInt8(truncatingIfNeeded: $0) }
            rankU8.withUnsafeBytes { handle.write(Data($0)) }
